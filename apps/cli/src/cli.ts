...
Commands:
  ...
  supervisor status      Show the local supervisor health and capabilities
	...
	contract evaluate      Evaluate a YAML contract against retained evidence
...
export async function runCli(environment: CliEnvironment): Promise<number> {
	...
	switch (command) {
		...
		case 'sessions':
			...
			writeLine(environment.stderr, 'Usage: agent-contract sessions list');
			return ExitCode.InvalidInvocation;
		case 'logs':
			return logsCommand(context, positionals[1]);
		case 'contract':
			return contractCommand(context, positionals[1], positionals[2], positionals[3]);
		default:
