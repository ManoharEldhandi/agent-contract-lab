...
test('run sends an explicit executable, argument vector, and isolation mode to the supervisor', async () => {
	let requestBody: unknown;
	const apiFetch: FetchLike = async (input, init) => {
		assert.equal(new URL(input).pathname, '/v1/runs');
		assert.equal(init?.headers?.['x-agent-contract-token'], 'test-token');
	...
	const result = await run(['run', '--format', 'json', '--actor', 'test-agent', '--isolated', '--', 'node', '-e', 'process.exit(0)'], apiFetch, { AGENT_CONTRACT_TOKEN: 'test-token' });
	assert.equal(result.code, ExitCode.Ok);
...
test('logs renders a readable timeline and one canonical event per JSONL line', async () => {
	...
	const pretty = await run(['logs', 'ses_demo', '--format', 'pretty'], apiFetch, { AGENT_CONTRACT_TOKEN: 'test-token' }, true);
	...
	const jsonl = await run(['logs', 'ses_demo', '--format', 'jsonl'], apiFetch, { AGENT_CONTRACT_TOKEN: 'test-token' });
...
test('watch sends resumable filters and renders one committed event per JSONL line', async () => {
	...
	const result = await run(['watch', 'ses_demo', '--after-sequence', '2', '--kind', 'file.changed,command.completed', '--grade', 'model-declared', '--actor', 'demo-agent', '--path', 'src/app', '--command', 'npm test', '--once', '--format', 'jsonl'], apiFetch, { AGENT_CONTRACT_TOKEN: 'test-token' });
	assert.equal(result.code, ExitCode.Ok);
...
test('cost renders a versioned provider cost report from the supervisor', async () => {
	...
	const result = await run(['cost', 'ses_demo', '--format', 'pretty'], apiFetch, { AGENT_CONTRACT_TOKEN: 'test-token' }, true);
	assert.equal(result.code, ExitCode.Ok);
...
test('export writes a supervisor-generated bundle and verify delegates manifest checking', async () => {
	const directory = await mkdtemp(join(tmpdir(), 'agent-contract-cli-export-'));
	const outputPath = join(directory, 'session.bundle.json');
	const bundle = { schemaVersion: 1, kind: 'agent-contract-evidence-bundle', manifest: { contentSha256: 'abc' } };
	try {
		...
		const exported = await run(['export', 'ses_demo', '--output', outputPath, '--format', 'json'], apiFetch, { AGENT_CONTRACT_TOKEN: 'test-token' });
		...
		const verified = await run(['verify', outputPath, '--format', 'json'], apiFetch, { AGENT_CONTRACT_TOKEN: 'test-token' });
...
test('contract evaluate sends YAML to the supervisor and returns Unknown for evidence gaps', async () => {
	const directory = await mkdtemp(join(tmpdir(), 'agent-contract-cli-test-'));
	const contractPath = join(directory, 'contract.yaml');
	await writeFile(contractPath, 'version: 1\nname: path-check\n', 'utf8');
	try {
		let body: unknown;
		const apiFetch: FetchLike = async (input, init) => {
			assert.equal(new URL(input).pathname, '/v1/sessions/ses_demo/contracts/evaluate');
			body = JSON.parse(init?.body ?? '{}');
			return new Response(JSON.stringify({
				schemaVersion: 1,
				contractName: 'path-check',
				decisions: [{ schemaVersion: 1, decisionId: 'dec_1', sessionId: 'ses_demo', contractName: 'path-check', ruleId: 'paths.allow[0]', result: 'unknown', message: 'Cannot determine whether changed paths are allowed.', evidenceEventIds: [], evaluatedAt: '2026-09-15T10:00:00.000Z', unknownReason: 'not-observed' }],
			}), { status: 200 });
		};
		const result = await run(['contract', 'evaluate', 'ses_demo', contractPath, '--format', 'pretty'], apiFetch, { AGENT_CONTRACT_TOKEN: 'test-token' }, true);
		assert.equal(result.code, ExitCode.Unknown);
		assert.deepEqual(body, { contractYaml: 'version: 1\nname: path-check\n' });
		assert.match(result.stdout, /Contract path-check/);
		assert.match(result.stdout, /evidence gap: not-observed/);
	} finally {
		await rm(directory, { recursive: true, force: true });
	}
});
