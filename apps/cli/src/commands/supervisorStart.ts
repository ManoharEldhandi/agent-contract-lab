...
import { launchDetachedSupervisor } from '@agent-contract-lab/local-supervisor';
...
/** Starts the packaged supervisor only after validating the loopback target. */
export async function supervisorStartCommand(context: CommandContext, dependencies: SupervisorStartDependencies = {}): Promise<ExitCode> {
	...
	try {
		(dependencies.launch ?? launchDetachedSupervisor)({ host, port });
	} catch (error) {
