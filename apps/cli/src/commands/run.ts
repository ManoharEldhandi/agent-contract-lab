...
export async function runCommand(context: CommandContext, command: readonly string[], actor: string | undefined, isolated: boolean): Promise<ExitCode> {
	const executable = command[0];
	if (executable === undefined) {
		writeLine(context.stderr, 'Usage: agent-contract run [--actor <name>] -- <executable> [arguments...]');
		return ExitCode.InvalidInvocation;
	}
	const outcome = await requestSupervisor<SessionResult>(context, '/v1/runs', 'POST', {
		workspacePath: process.cwd(), executable, args: command.slice(1), isolated, ...(actor === undefined ? {} : { actor }),
	});
	...
	if (context.format === 'pretty') {
		...
		writeLine(context.stdout, `Run 'agent-contract logs ${outcome.data.session.sessionId}' to inspect its committed evidence.`);
