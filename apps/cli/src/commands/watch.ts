...
import type { EventKind, SessionEvent } from '@agent-contract-lab/event-schema';
...
export async function watchCommand(context: CommandContext, sessionId: string | undefined, options: WatchOptions): Promise<ExitCode> {
	if (sessionId === undefined) {
		writeLine(context.stderr, 'Usage: agent-contract watch <session-id> [--once] [--after-sequence <n>]');
