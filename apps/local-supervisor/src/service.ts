...
import { evaluateContract, parseContract } from '@agent-contract-lab/policy-engine';
...
function eventMatchesQuery(event: import('@agent-contract-lab/event-schema').SessionEvent, query: EventQuery): boolean {
...
function isContainedPath(parent: string, child: string): boolean {
	const containedPath = relative(parent, child);
	return containedPath !== '' && !containedPath.startsWith('..') && !containedPath.includes('/../');
}

async function createIsolatedWorktree(dataDirectory: string, workspacePath: string): Promise<IsolatedWorktree | undefined> {
	const snapshot = await captureGitSnapshot(workspacePath);
	if (!snapshot.available) {
		return undefined;
	}
	const root = join(dataDirectory, 'worktrees');
	const target = join(root, `run_${randomUUID()}`);
	await mkdir(root, { recursive: true, mode: 0o700 });
	const created = await runGit(workspacePath, ['worktree', 'add', '--detach', target, 'HEAD'], 8 * 1024);
	...
	if (!isContainedPath(resolvedRoot, resolvedTarget)) {
		await runGit(workspacePath, ['worktree', 'remove', '--force', target], 8 * 1024);
		return undefined;
	}
	return { sourceWorkspacePath: workspacePath, workspacePath: resolvedTarget };
}

function policyPayload(decision: import('@agent-contract-lab/event-schema').PolicyDecision): JsonObject {
	return {
		decisionId: decision.decisionId,
		contractName: decision.contractName,
		ruleId: decision.ruleId,
		result: decision.result,
		message: decision.message,
		evidenceEventIds: [...decision.evidenceEventIds],
		...(decision.unknownReason === undefined ? {} : { unknownReason: decision.unknownReason }),
	};
}
...
export class SupervisorService {
	...
	async handle(request: ApiRequest): Promise<ApiResult> {
		if (!tokensMatch(this.authToken, request.authorization)) {
			return errorBody('unauthorized', 'a valid x-agent-contract-token header is required');
		}
		...
		try {
			...
			if (operation === 'contracts/evaluate' && request.method === 'POST') {
				return this.evaluateContract(sessionId, request.body);
			}
	...
	private async startRun(body: unknown): Promise<ApiResult> {
		...
		const worktree = parsed.isolated ? await createIsolatedWorktree(this.dataDirectory, workspace.workspacePath) : undefined;
		...
		if (worktree !== undefined) {
			await this.ledger.append(session.sessionId, {
				...
				payload: { isolated: true, sourceWorkspace: workspace.label },
			});
		}
	...
	private async evaluateContract(sessionId: string, body: unknown): Promise<ApiResult> {
		...
		const value = asRecord(body);
		const contractYaml = value === undefined ? undefined : requiredString(value, 'contractYaml');
		if (contractYaml === undefined) {
			throw new Error('contractYaml must be a non-empty YAML string');
		}
		const contractResult = parseContract(parseYaml(contractYaml, { maxAliasCount: 0 }));
		if (!contractResult.ok) {
			throw new Error(`invalid contract: ${contractResult.issues.join('; ')}`);
		}
		const decisions = evaluateContract(contractResult.contract, {
			sessionId,
			events: await this.ledger.listEvents(sessionId),
			now: this.now,
		});
		for (const decision of decisions) {
			await this.ledger.append(sessionId, {
				...
				payload: policyPayload(decision),
			});
		}
		return ok({ schemaVersion: 1, contractName: contractResult.contract.name, decisions });
	}
	...
	private async execute(sessionId: string, workspacePath: string, body: RunBody, worktree?: IsolatedWorktree): Promise<void> {
		...
		try {
			...
			await this.removeIsolatedWorktree(sessionId, correlationId, worktree);
	...
	private async removeIsolatedWorktree(sessionId: string, correlationId: string, worktree: IsolatedWorktree | undefined): Promise<void> {
		if (worktree === undefined) {
			return;
		}
		const removed = await runGit(worktree.sourceWorkspacePath, ['worktree', 'remove', '--force', worktree.workspacePath], 8 * 1024);
		if (removed.exitCode === 0) {
			await this.ledger.append(sessionId, {
				kind: 'worktree.removed', actor: 'supervisor', evidenceGrade: 'observed-boundary', correlationId, payload: { isolated: true },
			});
			return;
		}
		await this.ledger.append(sessionId, {
			kind: 'worktree.removed', actor: 'supervisor', evidenceGrade: 'unknown', unknownReason: 'adapter-error', correlationId,
			payload: { isolated: true, message: 'The supervisor could not remove the isolated worktree.' },
		});
	}
	...
	private parseRunBody(body: unknown): RunBody {
		...
		const isolated = value.isolated ?? false;
		if (workspacePath === undefined || executable === undefined || args === undefined || typeof isolated !== 'boolean') {
