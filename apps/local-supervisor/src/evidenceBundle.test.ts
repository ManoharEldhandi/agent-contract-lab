...
import type { SessionEvent, SessionRecord } from '@agent-contract-lab/event-schema';
...
function event(sequence: number): SessionEvent {
	return {
		...
		kind: sequence === 1 ? 'agent.summary' : 'policy.decision', actor: 'test-agent', evidenceGrade: sequence === 1 ? 'model-declared' : 'computed',
		payload: sequence === 1 ? { summary: 'completed' } : { decisionId: 'dec_1', result: 'pass' },
...
test('creates a stable canonical bundle from out-of-order retained events', () => {
	...
	assert.deepEqual(first.policyDecisions.map((entry) => entry.sequence), [2]);
