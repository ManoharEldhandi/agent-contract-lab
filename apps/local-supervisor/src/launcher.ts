...
/** Starts the packaged supervisor as a detached, loopback-only local process. */
export function launchDetachedSupervisor(options: DetachedSupervisorOptions): ChildProcess {
	...
	const child = spawn(process.execPath, [supervisorEntrypointPath()], {
		...
		stdio: 'ignore',
		windowsHide: true,
		env: { ...(options.environment ?? process.env), AGENT_CONTRACT_SUPERVISOR_HOST: options.host, AGENT_CONTRACT_SUPERVISOR_PORT: String(options.port) },
	});
	child.unref();
	return child;
}
