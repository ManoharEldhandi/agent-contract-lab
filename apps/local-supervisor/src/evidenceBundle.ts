...
import type { JsonObject, JsonValue, SessionEvent, SessionRecord } from '@agent-contract-lab/event-schema';
...
export const EVIDENCE_BUNDLE_KIND = 'agent-contract-evidence-bundle' as const;

const SECTION_NAMES = ['session', 'events', 'policyDecisions', 'workspaceDiffs', 'cost'] as const;
...
export interface EvidenceBundle {
	...
	readonly policyDecisions: readonly SessionEvent[];
...
export function createEvidenceBundle(session: SessionRecord, events: readonly SessionEvent[], generatedAt: string): EvidenceBundle {
	...
	const rawContent: JsonObject = {
		...
		policyDecisions: orderedEvents.filter((event) => event.kind === 'policy.decision') as unknown as JsonValue,
