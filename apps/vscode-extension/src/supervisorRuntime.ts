...
export function supervisorEnvironment(url: URL, environment: NodeJS.ProcessEnv = process.env): NodeJS.ProcessEnv {
	...
	return {
		...
		AGENT_CONTRACT_SUPERVISOR_HOST: host === 'localhost' ? '127.0.0.1' : host,
		AGENT_CONTRACT_SUPERVISOR_PORT: port,
	};
}

/** Owns only this extension's detached packaged-supervisor process handle. */
export class SupervisorRuntime {
	...
	async ensureRunning(url: URL): Promise<SupervisorConnection> {
		...
		if (this.child === undefined || this.child.exitCode !== null) {
			...
			this.child.once('error', (error) => { this.startupFailure = error instanceof Error ? error.message : String(error); });
