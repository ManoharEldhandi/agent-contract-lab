...
import { isJsonValue, type EventKind, type JsonObject } from '@agent-contract-lab/event-schema';
...
export interface AdapterEventSink {
	...
	unknown(kind: EventKind, reason: 'unsupported-capability' | 'not-observed' | 'ambiguous' | 'adapter-error', payload?: JsonObject): Promise<unknown>;
...
/**
 * Maps documented Claude Code hook payloads into one Agent Contract session.
...
/** Maps documented Codex App Server notifications into one Agent Contract session. */
		...
		if (method === 'item/started' || method === 'item/completed') {
			...
			if (['mcpToolCall', 'dynamicToolCall', 'collabToolCall', 'webSearch'].includes(itemType)) {
				...
				await record(method === 'item/started' ? 'tool.called' : 'tool.completed', {
					tool,
					...(method === 'item/started' ? { arguments: objectValue(item.arguments) } : { success: item.status === 'completed' || item.success === true }),
				}, itemCorrelation);
				return { emitted, completed: false };
			}
